...
interface ResultsViewProps {
    ...
    onStartReview: () => void;
    onViewAttemptHistory: () => void; // New prop
}

const ResultsView: React.FC<ResultsViewProps> = ({ examData, userAnswers, attempt, history, onTryAgain, onGenerateStudyPlan, onStartReview, onViewAttemptHistory }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-gray-200 dark:border-slate-700/50 rounded-xl p-6 mb-8 text-center shadow-lg">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Resultados do Exame</h1>
                <p className="text-lg text-gray-500 dark:text-gray-400 mb-4">{examData.examName}</p>
                <div className="flex justify-center items-baseline gap-4">
                    <p className={`text-7xl font-bold ${scoreColor}`}>{attempt.score.toFixed(1)}%</p>
                    <p className="text-2xl text-gray-700 dark:text-gray-300">({attempt.correctAnswers} / {attempt.totalQuestions} corretas)</p>
                </div>
            </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8"> {/* Adjusted grid columns */}
                ...
                <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 border border-transparent text-base font-medium rounded-md text-white bg-amber-600 hover:bg-amber-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:text-gray-500 dark:disabled:text-gray-400 disabled:cursor-not-allowed disabled:shadow-none transition-all duration-200 shadow-lg hover:shadow-amber-600/30"
                >
                    <MagnifyingGlassIcon className="w-5 h-5" />
                    Revisar Exame
                </button>
                 <button onClick={onTryAgain} className="w-full flex items-center justify-center gap-2 px-4 py-3 border border-gray-300 dark:border-slate-600 text-base font-medium rounded-md text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors">
                    <ArrowPathIcon className="w-5 h-5" />
                    Gerar Mais Questões
                </button>
                <button onClick={onViewAttemptHistory} className="w-full flex items-center justify-center gap-2 px-4 py-3 border border-gray-300 dark:border-slate-600 text-base font-medium rounded-md text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors">
            ...
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-gray-200 dark:border-slate-700/50 rounded-xl p-6 shadow-lg">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Revisão das Questões</h2>
